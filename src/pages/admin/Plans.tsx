  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Planos</h2>
          <p className="text-zinc-500 dark:text-zinc-400">Gerencie os planos de assinatura disponíveis.</p>
        </div>
        {!editingId && (
      ...
      >
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Informações Básicas</h3>
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Preço Mensal</label>
                <div className="relative">
                  ...
                  />
                  <div className="absolute right-3 top-2.5 text-xs text-zinc-400">BRL</div>
                </div>
              </div>
              <div className="col-span-2 space-y-2">
                <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Descrição</label>
                <textarea
                  className="flex min-h-[80px] w-full rounded-md border border-input bg-zinc-50 px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-zinc-900 dark:border-zinc-800"
          ...
          {/* Limites */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Limites do Sistema</h3>
            <div className="grid gap-6 md:grid-cols-3">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Max. Usuários</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox"
                      className="rounded border-zinc-300 text-blue-600 focus:ring-blue-500 h-3 w-3"
                ...
                <div className="relative">
                  <Users className={`absolute left-3 top-2.5 h-4 w-4 ${editForm.max_users === -1 ? 'text-zinc-300' : 'text-zinc-400'}`} />
                  <Input 
                    type="number"
                    className={`pl-9 bg-zinc-50 dark:bg-zinc-900 ${editForm.max_users === -1 ? 'text-zinc-400' : ''}`}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Max. Produtos</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox"
                      className="rounded border-zinc-300 text-blue-600 focus:ring-blue-500 h-3 w-3"
                ...
                <div className="relative">
                  <Package className={`absolute left-3 top-2.5 h-4 w-4 ${editForm.max_products === -1 ? 'text-zinc-300' : 'text-zinc-400'}`} />
                  <Input 
                    type="number"
                    className={`pl-9 bg-zinc-50 dark:bg-zinc-900 ${editForm.max_products === -1 ? 'text-zinc-400' : ''}`}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Max. Notas</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox"
                      className="rounded border-zinc-300 text-blue-600 focus:ring-blue-500 h-3 w-3"
                ...
                <div className="relative">
                  <FileText className={`absolute left-3 top-2.5 h-4 w-4 ${editForm.max_invoices === -1 ? 'text-zinc-300' : 'text-zinc-400'}`} />
                  <Input 
                    type="number"
                    className={`pl-9 bg-zinc-50 dark:bg-zinc-900 ${editForm.max_invoices === -1 ? 'text-zinc-400' : ''}`}
          ...
          {/* Funcionalidades */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Funcionalidades</h3>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <LayoutList className="absolute left-3 top-2.5 h-4 w-4 text-zinc-400" />
                <Input 
                  value={newFeature}
              ...
              {(!editForm.features_json || (Array.isArray(editForm.features_json) && editForm.features_json.length === 0)) ? (
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {Array.isArray(editForm.features_json) && editForm.features_json.map((feature: string, i: number) => (
                    <div key={i} className="group flex items-center justify-between rounded-md border border-zinc-200 bg-white p-2.5 text-sm shadow-sm transition-all hover:border-blue-200 dark:border-zinc-800 dark:bg-zinc-950 dark:hover:border-blue-900">
                      ...
                      <Button 
                        type="button" 
                        variant="ghost" 
                        className="h-6 w-6 p-0 text-zinc-400 opacity-0 transition-opacity hover:text-red-500 group-hover:opacity-100"
          ...
          <div className="flex items-center gap-8 rounded-lg bg-zinc-50 p-4 dark:bg-zinc-900">
            <label className="flex cursor-pointer items-center gap-3">
              <div className={`flex h-10 w-10 items-center justify-center rounded-lg transition-colors ${editForm.is_featured ? 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-zinc-200 text-zinc-400 dark:bg-zinc-800'}`}>
                <Star className="h-5 w-5" fill={editForm.is_featured ? "currentColor" : "none"} />
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Plano em Destaque</span>
                <span className="text-xs text-zinc-500">Aparecerá com ênfase na lista</span>
              </div>
              <input 
                type="checkbox" 
                className="hidden"
            ...
            <label className="flex cursor-pointer items-center gap-3">
              <div className={`flex h-10 w-10 items-center justify-center rounded-lg transition-colors ${editForm.is_active ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'bg-zinc-200 text-zinc-400 dark:bg-zinc-800'}`}>
                <Power className="h-5 w-5" />
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Status do Plano</span>
                <span className="text-xs text-zinc-500">{editForm.is_active ? 'Visível para clientes' : 'Oculto para clientes'}</span>
              </div>
              <input 
                type="checkbox" 
                className="hidden"
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {plans.map((plan) => (
          <Card key={plan.id} className={`flex flex-col overflow-hidden transition-all hover:shadow-lg ${!plan.is_active ? 'opacity-60 grayscale' : ''} ${plan.is_featured ? 'border-blue-500 ring-1 ring-blue-500/20' : ''}`}>
            <CardHeader className="flex flex-row items-start justify-between bg-zinc-50/50 pb-6 dark:bg-zinc-900/50">
              <div className="space-y-1">
                ...
                {plan.description && (
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 line-clamp-2 min-h-[40px]">{plan.description}</p>
                )}
              </div>
            ...
            <CardContent className="flex-1 space-y-6 pt-6">
              <div className="grid grid-cols-3 gap-2 text-center text-sm">
                <div className="rounded-lg bg-zinc-50 p-2 dark:bg-zinc-900">
                  ...
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Usuários</div>
                </div>
                <div className="rounded-lg bg-zinc-50 p-2 dark:bg-zinc-900">
                  ...
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Produtos</div>
                </div>
                <div className="rounded-lg bg-zinc-50 p-2 dark:bg-zinc-900">
                  ...
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Notas/mês</div>
                </div>
              </div>
